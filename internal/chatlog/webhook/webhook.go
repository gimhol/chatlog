...
func (m *MessageWebhook) Do(event fsnotify.Event) {
	...
	o.StartTime = m.lastTime
	o.EndTime = time.Now().Add(time.Minute * 10)
	o.Talker = m.conf.Talker
	o.Sender = m.conf.Sender
	o.Keyword = m.conf.Keyword
	o.Limit = 0
	o.Offset = 0
	messages, err := m.db.GetMessages(*o)
	if err != nil {
